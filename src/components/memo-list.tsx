...
const MemoItem: React.FC<MemoItemProps> = ({ memo, selectedMemoId, onSelectMemo, loggedInUser, tab, onUpdate, setMemos }) => {
    ...
    const handleMarkAsRead = async (memo: DashboardMemo) => {
        ...
        setMemos(prevMemos => prevMemos.map(m => {
            if (m.id === memo.id) {
                return { ...m, activity: [...m.activity, { actorId: actorId, action: 'viewed' as const }]};
            }
            return m;
        }));
    ...
    const getDisplayName = (memo: DashboardMemo) => {
        if (tab === 'sent' || tab === 'drafts' || tab === 'scheduled' || (tab === 'favorites' && memo.fromId === loggedInUser.id)) {
            if (memo.to.length > 0) {
                ...
                const otherCount = memo.to.length - 1 + memo.cc.length;
                return otherCount > 0 ? `${mainRecipient}, +${otherCount}` : mainRecipient;
            }
    ...
    const MemoActions = () => {
                                ...
                                <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                        <AlertDialogHeader>
                                            ...
                                            <AlertDialogDescription>This action cannot be undone. This will permanently delete this draft.</AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
            ...
            return (
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
                    <div className="bg-background/90 backdrop-blur-md rounded-full shadow-lg border border-border p-1 flex items-center gap-1">
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-gray-100" onClick={(e) => handleActionClick(e, () => handleArchive(memo.id, false))}>
        ...
        const canReply = isDirectRecipient && memo.fromId !== loggedInUser.id && (!loggedInUser.actingUser || loggedInUser.delegationPermissions?.includes('delegation:reply'));
        ...
        return (
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
                <div className="bg-background/90 backdrop-blur-md rounded-full shadow-lg border border-border p-1 flex items-center gap-1">
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-gray-100" onClick={(e) => handleActionClick(e, () => handleArchive(memo.id, true))}>
    ...
    return (
        <ContextMenu>
            <ContextMenuTrigger>
                ...
                >
                    <div className="flex w-full items-start justify-between gap-3">
                        ...
                        <div className="text-[10px] sm:text-xs shrink-0 text-muted-foreground whitespace-nowrap pt-0.5 font-medium">
                    ...
                    <div className="w-full relative">
                        <div className="pr-20 min-w-0 flex flex-col gap-0.5">
                            <div className={cn("text-sm truncate flex items-center gap-2", unread ? "font-semibold text-foreground" : "font-medium text-muted-foreground/90")}>
                                {showFavorite && (
                                    <button onClick={(e) => handleActionClick(e, () => handleToggleFavorite(memo.id))} className="shrink-0">
                                        <Star className={cn("h-4 w-4 transition-colors", isFavorited ? "fill-yellow-400 text-yellow-500" : "text-muted-foreground/40")} />
                                    </button>
                                )}
                                ...
                                <div className="flex flex-wrap gap-1 mt-1">
                                    {memo.labels.map(label => (
                                        <span 
                                            ...
                                            className="px-1.5 py-0 rounded-full text-[9px] font-bold border uppercase tracking-wider"
                                        >
                                            {label.name}
                                        </span>
                                    ))}
                            ...
                            <div 
                                className="line-clamp-1 text-xs text-muted-foreground/70 break-all mt-0.5" 
            ...
            <ContextMenuContent className="w-56">
                {tab === 'drafts' ? (
                    <ContextMenuItem onSelect={() => handleDeleteDraft(memo.id)} className="text-destructive">
        ...
        <div className="flex flex-col items-center gap-3 p-2">
            {memos.map((memo) => (
                 <Tooltip key={memo.id} delayDuration={0}>
                    <TooltipTrigger asChild>
                        <button
                            className={cn("relative rounded-full transition-transform active:scale-95", selectedMemoId === memo.id ? "ring-2 ring-primary ring-offset-2" : "hover:scale-105")}
                            onClick={() => onSelectMemo(memo.id)}
                        >
