  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const csvData = result.records.map((r: any) => ({
        'Onboarding ID': r.id,
        'Mnemonic': r.mnemonic.toUpperCase(),
        'Current Status': r.approvalStatus,
        'Full Name': `${r.givenName || ''} ${r.familyName || ''}`.trim().toUpperCase() || r.fullName1.toUpperCase(),
        'Short Name': r.shortName.toUpperCase(),
        'Title': r.title.toUpperCase(),
        'Gender': r.gender.toUpperCase(),
        'Date of Birth': r.dateOfBirth,
        'Nationality': r.nationality?.toUpperCase() || '',
        'Legal ID Number': r.legalIdNumber ? `="${r.legalIdNumber.toUpperCase()}"` : '',
        'National ID Number': r.nationalIDNumber ? `="${r.nationalIDNumber.toUpperCase()}"` : '',
        'PSU Token (Fayda)': r.psuToken ? `="${r.psuToken.toUpperCase()}"` : '',
