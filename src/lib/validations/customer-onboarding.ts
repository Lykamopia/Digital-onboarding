...
export const CustomerOnboardingSchema = z.object({
  ...
  // Channel routing — optional on the wire. Omit either one and the middleware
  // substitutes DEFAULT_ACCOUNT_OFFICER / DEFAULT_PRODUCT_TYPE server-side, so
  // no default is baked in here (this schema is also used in the browser).
  accountOfficer:     z.string().optional().nullable().or(z.literal('')),
  productType:        z.string().max(20).optional().nullable().or(z.literal('')),
